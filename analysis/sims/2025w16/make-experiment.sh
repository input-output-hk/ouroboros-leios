...
#!nix-shell -i bash -p gzip remarshal
...
case "$SIMULATOR" in
  haskell)
    echo "../../ols sim short-leios --leios-config-file config.json --topology-file network.yaml --shared-log-format JSON --output-file sim.log --output-seconds $MAX_SLOT > stdout 2> stderr" >> "$RUN_DIR/run.sh"
  ;;
rust)
