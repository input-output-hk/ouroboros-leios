...
DELTA_HEADER=$(echo -n "$LABEL" | sed -e 's/^\(.*\),\(.*\),\(.*\),\(.*\)/\1/')
STAGE_LENGTH_VOTE=$(echo -n "$LABEL" | sed -e 's/^\(.*\),\(.*\),\(.*\),\(.*\)/\2/')
STAGE_LENGTH_DIFF=$(echo -n "$LABEL" | sed -e 's/^\(.*\),\(.*\),\(.*\),\(.*\)/\3/')
...
, "leios-header-diffusion-time-ms": '"$DELTA_HEADER"'
