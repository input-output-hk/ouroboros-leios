...
THROUGHPUT=0.150
LABEL=$(basename "$PWD")
KEEP=$(echo "$LABEL" | sed -e 's/^\(.*\),\(.*\),\(.*\)$/\1/')
EB_DELAY=$(echo "$LABEL" | sed -e 's/^\(.*\),\(.*\),\(.*\)$/\2/')
TX_ATTACK=$(echo "$LABEL" | sed -e 's/^\(.*\),\(.*\),\(.*\)$/\3/')
if [[ "$TX_ATTACK" == 0 ]]
then
...
.nodes |=
  map_values(
    .producers |=
      map_values(
        if ."latency-ms" <= 10 or .random <= '"$KEEP"' then
          .
        else
          empty
        end
...
../../topology-checker --input network.yaml --output network.md
...
$KEEP,33% adversary,EB attack: $(echo "$EB_DELAY s/EB" | sed -e 's@^0\.0 s/EB@none@'),Tx attack: $(echo "$((100 * TX_ATTACK))%" | sed -e 's@^0%@none@'),$TX_SIZE B/Tx,$THROUGHPUT TxMB/s,$TX_START,$TX_STOP,$SIM_STOP
EOI
