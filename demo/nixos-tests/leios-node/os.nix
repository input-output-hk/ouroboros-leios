...
}:
{
  imports = [
    (import ../services/simple-cardano-relay.nix {
      inherit (inputs.cardano-node-leios.packages."${pkgs.system}") cardano-node;
    })
  ];
  environment.systemPackages = [ inputs.cardano-node-leios.packages."${pkgs.system}".cardano-cli ];
  cardano.relay = {
    enable = true;
    config = ./config.json;
