...
def filter_log_events(log_path: str, filter_text: str, subfilter_text: str = ""):
    ...
    try:
        with open(log_path, "r") as f:
            for line in f:
                try:
                    ...
                    # TypeError: message field is not a string (e.g., integer like 3002)
                    # KeyError: message field doesn't exist or other required field missing
                    continue
                except Exception:
                    # Catch any other unexpected errors (e.g., AttributeError when message is not a dict)
                    # and continue processing the rest of the log
                    continue
        ...
        print(
            f"An unexpected error occurred while processing {log_path}", file=sys.stderr
        )
        raise


def create_and_clean_df(records: list, node_id: str) -> pd.DataFrame:
    """
...
def load_block_arrivals(df_headers: pd.DataFrame, log_path: str, node_id: str):
    ...
    df = create_and_clean_df(raw_data, node_id)[["hash", "at"]]
    ...
    df["latency_ms"] = (
        ((df["at"] - df["slot_onset"]).dt.total_seconds() * 1000)
        .round(0)
        .astype("Int64")
    )
...
def load_leios_block_arrivals(
    df_schedule: pd.DataFrame, log_path: str, node_id: str, subfilter: str
):
    """
    ...
    # Use 'Int64' to allow NaNs; see https://stackoverflow.com/a/54194908
    df["latency_ms"] = (
        ((df["at"] - df["offer"]).dt.total_seconds() * 1000).round(0).astype("Int64")
    )
...
empty_sendrecv = pd.DataFrame(
    columns=["at", "mux_at", "connection_id", "direction", "msg", "prevCount"]
)
        ...
        with open(log_path, "r") as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    # Skip lines that can't be parsed (e.g., process-compose noise)
                    continue
                records.append(record)
...
def join_sendrecv(
    left_connId: str, left_id: str, left: pd.DataFrame, rite_id: str, rite: pd.DataFrame
):
    df = pd.merge(
        left[left["connection_id"] == left_connId],
        rite[rite["connection_id"] == flipConnectionId(left_connId)].apply(
            lambda x: flips.get(x.name, noFlip)(x)
        ),
    ...
    df["send_latency_ms"] = (
        (
            abs(
                pd.to_datetime(df["send_at"], utc=True)
                - pd.to_datetime(df["push_at"], utc=True)
            ).dt.total_seconds()
            * 1000
        )
        .round(0)
    ...
    df["transport_latency_ms"] = (
        (
            abs(
                pd.to_datetime(df["recv_at"], utc=True)
                - pd.to_datetime(df["send_at"], utc=True)
            ).dt.total_seconds()
            * 1000
        )
    ...
    df["recv_latency_ms"] = (
        (
            abs(
                pd.to_datetime(df["pull_at"], utc=True)
                - pd.to_datetime(df["recv_at"], utc=True)
            ).dt.total_seconds()
            * 1000
        )
...
if __name__ == "__main__":
    ...
    df_headers = (
        create_and_clean_df(header_data, "node0")[["hash", "slot"]]
        .dropna(subset=["slot"])
    ...
    df_merged = pd.merge(
        ...
        on=[
            ...
            "slot_onset",
            "hash",
        ],  # note that this determines order of resulting rows
        how="outer",
    )
    ...
    df_leios_block_node0 = load_leios_block_arrivals(
        df_schedule, log_path_node0, "node0", LEIOS_BLOCK_EVENT_SUBFILTER
    )
    df_leios_block_downstream = load_leios_block_arrivals(
        df_schedule, log_path_downstream, "downstream", LEIOS_BLOCK_EVENT_SUBFILTER
    )

    # We also merge on "slot_onset" to retain it. It's a function of "slot", so
    # this is harmless.
    df_leios_block_merged = pd.merge(
        df_leios_block_node0,
        df_leios_block_downstream,
        suffixes=["_node0", "_downstream"],
        on=[
            "offer_slot",
            "offer",
    ...
    df_leios_blocktxs_node0 = load_leios_block_arrivals(
        df_schedule, log_path_node0, "node0", LEIOS_BLOCKTXS_EVENT_SUBFILTER
    )
    df_leios_blocktxs_downstream = load_leios_block_arrivals(
        df_schedule, log_path_downstream, "downstream", LEIOS_BLOCKTXS_EVENT_SUBFILTER
    )

    # We also merge on "slot_onset" to retain it. It's a function of "slot", so
    # this is harmless.
    df_leios_blocktxs_merged = pd.merge(
        df_leios_blocktxs_node0,
        df_leios_blocktxs_downstream,
        suffixes=["_node0", "_downstream"],
        on=[
            "offer_slot",
            "offer",
