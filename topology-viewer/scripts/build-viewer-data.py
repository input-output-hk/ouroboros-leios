...
# plausible location.
REGION_COORDS = {
    "aws:us-east-1": (-77.46, 38.95),
    "aws:us-east-2": (-83.00, 39.96),
    "aws:us-west-1": (-121.89, 37.34),
    "aws:us-west-2": (-122.68, 45.52),
    "aws:ca-central-1": (-73.55, 45.50),
    "aws:eu-west-1": (-6.27, 53.35),
    "aws:eu-west-2": (-0.13, 51.51),
    "aws:eu-west-3": (2.35, 48.86),
    "aws:eu-central-1": (8.68, 50.11),
    "aws:eu-central-2": (8.55, 47.37),
    "aws:eu-north-1": (18.06, 59.33),
    "aws:eu-south-1": (9.19, 45.46),
    "aws:ap-northeast-1": (139.69, 35.69),
    "aws:ap-northeast-2": (126.98, 37.57),
    "aws:ap-northeast-3": (135.50, 34.69),
    "aws:ap-south-1": (72.88, 19.08),
    "aws:ap-southeast-1": (103.85, 1.29),
    "aws:ap-southeast-2": (151.21, -33.87),
    "aws:af-south-1": (18.42, -33.93),
    "aws:sa-east-1": (-46.63, -23.55),
    "aws:me-south-1": (50.59, 26.07),
    "gcp:us-east1": (-80.93, 33.21),
    "gcp:us-east4": (-77.46, 38.95),
    "gcp:us-central1": (-95.86, 41.26),
    "gcp:us-west1": (-120.74, 45.60),
    "gcp:us-west2": (-118.24, 34.05),
    "gcp:us-west3": (-111.89, 40.76),
    "gcp:us-west4": (-115.14, 36.17),
    "gcp:europe-west1": (3.83, 50.45),
    "gcp:europe-west2": (-0.13, 51.51),
    "gcp:europe-west3": (8.68, 50.11),
    "gcp:europe-west4": (5.79, 53.44),
    "gcp:europe-west6": (8.55, 47.37),
    "gcp:europe-west9": (2.35, 48.86),
    "gcp:europe-north1": (24.93, 60.17),
    "gcp:europe-southwest1": (-3.70, 40.42),
    "gcp:asia-east1": (120.96, 23.69),
    "gcp:asia-east2": (114.16, 22.31),
    "gcp:asia-northeast1": (139.69, 35.69),
    "gcp:asia-northeast3": (126.98, 37.57),
    "gcp:asia-southeast1": (103.85, 1.29),
    "gcp:asia-southeast2": (106.85, -6.21),
    ...
    "azure:eastus": (-79.40, 37.20),
    "azure:eastus2": (-78.65, 36.85),
    "azure:westus": (-122.42, 37.78),
    "azure:westus2": (-119.85, 47.23),
    "azure:westus3": (-112.07, 33.45),
    "azure:northeurope": (-6.27, 53.35),
    "azure:westeurope": (4.89, 52.37),
    "azure:uksouth": (-0.13, 51.51),
    "azure:centralfrance": (2.35, 48.86),
    "azure:francecentral": (2.35, 48.86),
    "azure:germanywc": (8.68, 50.11),
    "azure:swedencentral": (17.64, 59.85),
    "azure:switzerlandn": (8.55, 47.37),
    "azure:koreacentral": (126.98, 37.57),
    "azure:southeastasia": (103.85, 1.29),
    "azure:centralindia": (73.86, 18.52),
    ...
    "ovh:de:saarland": (6.99, 49.40),
    "ovh:de:saarbrücken": (6.99, 49.24),
    "ovh:gb:london": (-0.13, 51.51),
    "ovh:gb:bexley": (0.15, 51.44),
    "ovh:ca:montreal": (-73.55, 45.50),
    "ovh:ca:beauharnois": (-73.88, 45.32),
    "ovh:ca:quebec": (-71.21, 46.81),
    "ovh:us:virginia": (-77.46, 38.95),
    "ovh:us:hillsboro": (-122.99, 45.52),
    "ovh:us:oakton": (-77.30, 38.89),
    "ovh:in:mumbai": (72.88, 19.08),
    "ovh:au:sydney": (151.21, -33.87),
    "ovh:sg:singapore": (103.85, 1.29),
    "ovh:pl:warsaw": (21.01, 52.23),
    "hetzner:falkenstein": (12.37, 50.48),
    "hetzner:helsinki": (24.93, 60.17),
    "hetzner:nuremberg": (11.08, 49.45),
    "hetzner:ashburn": (-77.46, 39.04),
    "hetzner:singapore": (103.85, 1.29),
    "contabo:lauterbourg": (8.18, 48.94),
    "contabo:st_louis": (-90.20, 38.63),
    "contabo:orangeburg": (-74.05, 41.01),
    "contabo:seattle": (-122.33, 47.61),
    "contabo:singapore": (103.85, 1.29),
    "contabo:tokyo": (139.69, 35.69),
    "contabo:sydney": (151.21, -33.87),
    "contabo:portsmouth": (-76.30, 36.84),
    "contabo:nuremberg": (11.08, 49.45),
    "contabo:karlsruhe": (8.40, 49.01),
    "contabo:frankfurt_am_main": (8.68, 50.11),
    "contabo:london": (-0.13, 51.51),
    "worldstream:naaldwijk": (4.20, 51.99),
    "leaseweb:haarlem": (4.65, 52.38),
}
...
def compute_spread(node_lon, node_lat, peer_lonlats):
    ...
    if not peer_lonlats:
        return 0.0
    dists = [
        haversine_km(node_lon, node_lat, plon, plat) for plon, plat in peer_lonlats
    ]
...
def process_topology(yaml_path, meta_path):
    ...
    if overridden:
        print(
            f"    overrode location for {overridden}/{len(nodes)} nodes "
            f"(BP centroid → region/capital)",
            file=sys.stderr,
        )
    ...
    for nid, info in nodes.items():
        ...
        spread_km = compute_spread(
            info["location"][0], info["location"][1], peer_lonlats
        )
        m = info["_meta"]
        if m["kind"] == "bp":
            n_bps += 1
...
def main():
    ...
    for stem in targets:
        ...
        print(
            f"  wrote {out_path.name} ({sz:.0f} KB)  "
