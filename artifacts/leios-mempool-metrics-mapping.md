...
5. **Skip-rate baselines shift with parameters.** Musashi runs gap 10 today, the CIP says 14; comparing skip rates across periods requires re-baselining (Nagel's exact point in the wording thread).
...
3. **Chain vs analytic skip**: kleioscan skip rate vs 1 − P_certified from ΔQ/`linleios` with the *measured* π₁ plugged in (and musashi's gap 10) — closing the measured-alignment → certification-probability pipeline end to end.
