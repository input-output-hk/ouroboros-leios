...
This document answers: **which parameters, and which inequalities over them, decide whether blocks (RBs and EBs), votes, and certificates are created and accepted or rejected** in the `leios-prototype` node — and where each parameter actually lives.
...
- **EB:** `ebCap = ebCapacityTxMeasure` = [`leiosEndorserBlockMeasure`](https://github.com/IntersectMBO/ouroboros-consensus/blob/b56977baae0740f563060a8a9171c78be865b357/ouroboros-consensus-cardano/src/shelley/Ouroboros/Consensus/Shelley/Ledger/Mempool.hs#L752), a `DijkstraMeasure` whose coordinates are all ledger parameters:
  ...
  - tx **count** ≤ `min(maxTxsPerEb^wire, maxEbTxCount(maxEndorserBlockReferencesSize))` — the references-size parameter converted to a count at ≥ 36 B per reference entry ([`maxEbTxCount`](https://github.com/IntersectMBO/ouroboros-consensus/blob/b56977baae0740f563060a8a9171c78be865b357/ouroboros-consensus/src/ouroboros-consensus/LeiosDemoTypes.hs#L2226)); musashi: `maxEbTxCount(100 kB) = 2,776` vs the wire bound 13,887, so the parameter governs.
...
|---|---|---|---|
| EB capacity caps (bytes / ExUnits / ref-scripts / count) | ✅ `leiosEndorserBlockMeasure` (all four ledger params) | ❌ FIXME | ❌ FIXME (`Forker.hs`) + TODO (`Praos.hs`); no ledger rule consumes the params |
...
| Announced size ≤ references-size param | ✅ implicitly via count clamp | ❌ | ❌ TODO `LeiosEbTooBig`; body checked only against the peer's own offer |

The pattern: **honest-producer inequalities are implemented; the corresponding Byzantine-facing acceptance checks are FIXME/TODO**, except the quorum-and-signature core of certificate verification, which is complete. This sharpens the scope-candidate conformance property (workstream (e)): the timing rule is only one of five header-level checks awaiting enforcement.

## 5. Deployed values (musashi, fetched 2026-09-17)

From `dijkstra-genesis.json` (hash `aa1238f5…`), `shelley-genesis.json` (slot = 1 s, epoch = 21,600 slots, f = 0.05, k = 108), and `config.json`:
...
| `maxEndorserBlockReferencesSize` | 100,000 B | `maxEbTxCount` ⇒ **≤ 2,776 txs/EB** (wire limit 13,887 not binding) |
...
| `MempoolCapacityBytesOverride` (node config) | 2,000,000 B | the 2× default is **overridden** on musashi; CIP's ≥ 2×(RB + EB-txs) at these params wants ≥ ~2.2 MB ❓🤖 |
...
- **Committee scheme:** the CIP now specifies **stake-based truncation** (top-N_c, deterministic — "quorum failure is not caused by sortition randomness"), with wFA^LS relegated to earlier drafts. Our previous documents' "wFA^LS committee" tags were stale against the current CIP and are corrected as of this document.
...
[CIP-0164 step 5](https://github.com/cardano-foundation/CIPs/blob/master/CIP-0164/README.md#specification) says a certificate may be included if `RB′` is **at least** `gap` slots after the announcing RB, i.e. `slot(RB′) − s ≥ gap`. The implementation's guard is
...
Worth flagging for anyone reading both literatures: the CIP quotes "0.95¹³ ≈ 51%" as the probability an EB **survives**, while our own notes quote ≈51% as the probability it is **skipped**. The two numbers are near-identical and mean opposite things. Our `1 − (1−f)^gap` formula happens to be exactly right for the deployed code, because the implementation's extra slot widens the window to `gap`.

### 7.2 Three of the CIP's six vote conditions are not enforced

[CIP-0164 step 3](https://github.com/cardano-foundation/CIPs/blob/master/CIP-0164/README.md#specification) lists six conditions under which a committee member votes. Against `runLeiosVoting`/`goVote` at this snapshot:

| CIP vote condition | Implementation |
|---|---|
| 1. The RB header arrived within L_hdr | **Not enforced.** The arrival delay is computed and passed into `processAnnouncementCentrally`, whose haddock says it is recorded "so the body/closure arrival handlers can report the EB's age" — telemetry only. |
...
Source read at `ouroboros-consensus @ b56977b`, `cardano-ledger @ 1587f21`, `cardano-base @ fbfb3f0` (all permalinked inline; ledger and base branch tips verified identical to the node's `cabal.project` pins on 2026-09-17); [CIP-0164](https://github.com/cardano-foundation/CIPs/blob/master/CIP-0164/README.md) (master, cached 2026-09-16); musashi configs from book.play.dev.cardano.org fetched 2026-09-17 (saved to the session scratchpad).
