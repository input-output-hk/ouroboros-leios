...
if (searchInput && searchResults) {
  ...
  function generateSearchResults(results) {
    return results.map(result => {
      ...
      return \`
        <a href="\${result.path}#L\${result.lineNumber}" class="search-result" data-line="\${result.lineNumber}">
          <h3 class="search-header search-identifier">
            <span>\${highlightedTitle}</span>
            <span class="search-module">\${result.moduleName}</span>
          </h3>
          <span class="search-type sourceCode">\${result.content}</span>
        </a>
      \`;
    }).join('');
  }
