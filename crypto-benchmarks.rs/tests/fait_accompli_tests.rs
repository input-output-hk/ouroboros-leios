...
fn fa_test() {
  ...
  assert_eq!(1, fa.n_nonpersistent, "Incorrect number of non-persistent voters.");
  assert_eq!(Ratio::from_integer(BigInt::from(100)), fa.rho, "Incorrect rho.");
  ...
  let expected_persistent: Vec<(PoolKeyhash, Ratio<BigInt>)> = 
     vec![(p1, 3000u64), (p2, 500u64), (p3, 200u64), (p4, 100u64)]
       ...
       .collect();
  assert_eq!(expected_persistent, fa.persistent, "Unexpected persistent voters.");
  ...
  let expected_nonpersistent: Vec<(PoolKeyhash, Ratio<BigInt>)> =
    vec![(p8, 10u64), (p5, 100u64), (p6, 50u64), (p7, 40u64)]
       ...
       .collect();
  let actual_nonpersistent: Vec<(PoolKeyhash, Ratio<BigInt>)> = fa.nonpersistent.into_iter().collect();
  assert_eq!(expected_nonpersistent, actual_nonpersistent, "Unexpected non-persistent voters.");
