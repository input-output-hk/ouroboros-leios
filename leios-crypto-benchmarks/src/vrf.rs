use blst::{blst_sk_to_pk_in_g1, blst_p1_cneg, blst_bendian_from_scalar, blst_fr, blst_fr_add, blst_fr_from_scalar, blst_fr_mul, blst_hash_to_g1, blst_p1, blst_p1_add, blst_p1_generator, blst_p1_mult, blst_scalar, blst_scalar_from_bendian, blst_scalar_from_fr, blst_sha256, min_pk::{PublicKey, SecretKey}};
use rand::RngCore;
...
pub fn vrf_prove(sk : &SecretKey, input : &[u8], dst : &[u8]) -> (blst_p1, blst_scalar, blst_fr) {
  ...
  unsafe {
    ...
    blst_fr_mul(&mut csk, &mut csk, &sk_fr);
...
pub fn vrf_verify(pk : &blst_p1, input : &[u8], dst : &[u8], gamma : &blst_p1, c : &blst_scalar, s : &blst_fr) -> bool {
  ...
  unsafe {
    ...
    blst_p1_mult(&mut sgg, blst_p1_generator(), s_bytes.as_ptr(), 8 * s_bytes.len());
