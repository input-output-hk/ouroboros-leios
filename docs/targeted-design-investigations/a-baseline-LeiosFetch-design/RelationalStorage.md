...
DELETE FROM EB2TX
WHERE ebSlot < TheCurrentCutoff
RETURNING txHash;    # TODO need to populate the TEMP table via SELECT, then DELETE according to TEMP table.
...
(My first interpretation of the source code comments on `struct PCache` in `pcache.c` is that a "use" is when a cached page _first_ becomes dirty.
However, I more carefully searched the C code for manipulations of pDirtyNext and now I think it's whenere a transaction that wrote to the page commits.)
