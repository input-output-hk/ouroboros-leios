...
TODO This document successfully elicited feedback, but it has not all been integrated.
See https://github.com/input-output-hk/ouroboros-leios/pull/797 for the unresolved Comments.
...
- Don't request the same transaction from more than LeiosFetchMultiplicity-many peers at a time.
    ...
    - See the [InFlightMap](#InFlightMap) below.
...
- Don't request transactions that were already fetched when fetching a "recent" EB.
    - CIP-0164 does not elaborate on "recent", but some analysis of protocol during a mempool fragmentation scenario suggests only the freshest ~100 EBs are "recent".
    - See the [TxCache](#TxCache) below.
...
- Consider the set of all EBs that are still missing some transactions, are still younger than the immutable tip, and have been offered by some of the current peers.
  The youngest such EB is always the top priority of LeiosFetch---a.k.a. FreshestFirst.
    ...
    - Otherwise, prioritize younger EBs over older EBs.
    - TODO should we "somewhat prioritize" youngest EBs that have been announced even before they've been offered?
- However, that top priority EB across all peers should not be the _exclusive_ priority of LeiosFetch across all peers.
    - If that EB is offered by an adversarial peer, they can simple choose to withhold it.
    - Doing so should not cause the LeiosFetch logic to significantly delay issuing any requests to its (honest) peers that have not already offered the top priority EB.
    - In other words, the adversary should not have undue influence on the LeiosFetch decision logic.
- Don't waste time (i.e. no unnecessary latency beyond the above).
...
- The node maintains the WipEBs themselves on-disk.
  Again, ~10000 is small enough that there could simply be one file per EB, with the slot-and-hash as its filename.
    - One key remark: we would only need `fsync` calls when shutting down the node.
    - In particular, the primary requirements and architecture do not require other concurrent OS processes to also observe our node's alterations of these files.
...
If a single atomic EbBody message proves untenable, we can break it into pieces, using tools like Merkle trees for cost-effective integrity checks.

The centralized LeiosFetch decision logic will iterate at 10 Hz or more---it'll react to incoming messages while respecting a debouncing rate-limit.
...
- Acquire a Mempool snapshot (see [TxCache](#TxCache) below for a compare and contrast).
...
The TxCache and the Mempool are both places that the LeiosFetch logic can find a transaction instead of needing to fetch it from a peer, avoiding network latency.
It is useful to distinguish them because their contents serve different purposes, with correspondingly different policies for eviction etc.
    - The Mempool is the staging area for transactions to be included in the next issued block.
      It's also using by TxSubmission so that other nodes could include those transactions.
      Transactions are evicted when the node's selection changes such that those transactions are no longer valid.
      Transactions are added when the downstream peers supply them.
      It is capacity is limited and results in back-pressure to user programs submitting transactions to the whole network.
    - The TxCache instead ensures that the validation and diffusion work done for an EB that does not end up on chain will almost certainly be reused to accelerate the next EB.
      In particular, during a mempool fragmentation, all EBs issued by one clique of stake will overlap between successes.
      If they have a non-trivial amount of stake, then the whole network's TxCaches will not need to diffuse or fully validate the overlap for each such EB.
