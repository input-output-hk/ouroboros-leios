...
def extend_database(db_name, num_ebs):
    ...
    print(f"For 10000 EBs, this generated a ~27.5 GB file and took about ~30 minutes with a nice-ish NVMe drive.")
    print("")
    
    tx_indices = np.arange(15000, dtype=int)
    
    eb_data2 = [None] * rows_to_insert
    for i, (slot, e_hash) in tqdm.tqdm(enumerate(eb_data), desc="Generating", miniters=1, unit="", total=rows_to_insert, smoothing=0.01):
        ...
        while True:
            ...
            if True: # 11995000 <= total <= 12000000:   # FYI, replacing this with True was about 17x faster
                break
        ...
        eb_data2[i] = list(   (slot, e_hash, int(idx), int(size)) for idx, size in zip(tx_indices, sizes)   )
        
    sqltime3 = time.time()
    for i, batch in tqdm.tqdm(enumerate(eb_data2), desc="Writing", miniters=1, unit="", total=rows_to_insert, smoothing=0.01):
        cursor.executemany('''
    ...
    print("")
    print(f"Completed INSERTs INTO table EB2TX at {datetime.datetime.now().strftime("%c")}")
    ...
    conn.close()
    print("")
    print(f"Done creating/updating {db_name} after ({sqltime2 - sqltime1:.1f}, {sqltime4 - sqltime3:.1f}, {sqltime6 - sqltime5:.1f})")
