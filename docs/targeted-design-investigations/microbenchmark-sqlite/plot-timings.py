...
def plot_interactive_flipbook(file_pattern, upper_bound):
    ...
    line1, = ax.plot(initial_time, all_col1[0], color=color_col1, lw=2, label='1st SELECT')
    line2, = ax.plot(initial_time, all_col2[0], color=color_col2, alpha=0.5, lw=2, label='2nd SELECT')
    ...
    if padding == 0: padding = 1.0 # Edge case fallback if all data is perfectly flat
