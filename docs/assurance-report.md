...
| R3 | Chain validity and selection unchanged | P, C | P-SAFETY, P-LIVE, P-INST, P-PROG, C-VERIF | Partial |
...
**R3 — Chain validity and selection unchanged.** The strongest formal results in the project: P-SAFETY and P-LIVE prove *transfer* theorems (if Praos satisfies Common Prefix / Honest Chain Growth / Existential Chain Quality, then Leios-as-extension does), and P-INST instantiates them for Linear Leios (`leiosSafety`, `leiosHCG`, `leios∃CQ`). C-VERIF checks per-node behaviour treats EBs/votes as auxiliary. P-PROG closes a vacuity hole in all of the above: the transfer theorems are invariant-style statements (preservation along traces), which would say nothing of a relation that can wedge; P-PROG proves the bare node completes every slot — every future slot is reachable (`enough-traces`) — for arbitrary delivered messages, so the quantified trace set is non-trivial. **Missing for discharge:** proofs (or documented reductions to standard assumptions) for the six open hypotheses the instantiation is parameterised on — `HashCorrectB`, `hash-unique`, `isPure-Leios`, `IsBlockchain-base`, `is-extension-eq`, and the per-deployment Chain/Slot lemmas. Six obligations; none currently discharged. Additionally, P-PROG covers the bare node only — the progress lemma for the composite node (node with network shim and base layer) is stated there as future work (W18).
...
| W9 | S | R1, R5 | Max-load and withheld-then-released-burst campaigns measuring RB time-to-≥95 %-of-stake against the 5 s budget, with honest-cost-per-attacker-byte accounting in the burst case. Instrument available: `leios-tools` eb-burst adversarial behaviour (T23). |
...
| W16 | C, T | R16 | Extend the trace verifier to R16: the spec currently states no serving/retention obligation (diffusion is abstracted into FFD) and the verifier's actions are produce/acquire-side only, so add the obligation as spec premises or verifier side-conditions and consume the `EBSent` events the trace schema already defines; complement with a two-node integration test requesting EBs the SUT did not vote for and does not prefer. |
...
| W18 | P | R3 | Extend P-PROG's progress proof from the bare node to the composite node (node with network shim and base layer), as flagged in `Leios/Linear/Progress.agda`. |
