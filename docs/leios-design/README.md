...
The guidelines below inform how the new components in the diagram are structured internally and how they integrate with the existing consensus codebase. They are not Leios-specific, but articulate a discipline that consensus extensions in general could follow.

#### Inertness is structural, not gated

The ledger is the single source of truth for which features are active, which parameters apply, and who participates. Code ideally does not carry a parallel opinion. A Leios-capable node running in a pre-Leios era constructs every state store, runs every thread, and registers every mini-protocol exactly as it would post-fork. Those components consult the ledger, find no committee or no eligible round, and produce no work, no traffic, and no state changes. When the ledger transitions to Leios at an epoch boundary, the same code begins producing effects, because its ledger queries now return data. Nothing switches on; nothing is spawned for the first time. The canonical idiom is a ledger query whose result drives behavior — for example a `getCurrentCommittee :: LedgerState blk -> Maybe Committee` returning `Nothing` pre-Leios, with all consumers short-circuiting on `Nothing`. The era logic is encoded once, in the ledger; consuming code is symmetric across eras.

Don't gate thread spawning, mini-protocol registration, or state-store creation on era status; wire all three up unconditionally at startup, even if their backing components have nothing to do until the ledger says otherwise. And don't dispatch runtime behavior between "old" and "new" code paths based on era — there is one behavior, and the ledger decides what it produces.

The trade-off is a steady-state cost: idle threads parked on STM, empty caches, and registered-but-quiet channels on every long-lived peer connection — paid universally, including by nodes that sync only historical chains and will never see Leios active. We accept it in exchange for a single source of truth and a hard fork that is purely a ledger event, not a code path. The guideline has prior art in consensus: the Hard Fork Combinator runs one binary across all Cardano eras with era-awareness pushed into ledger types rather than dispatch code, and the mempool validates against the current ledger without an era switch. Coarse-grained switches that decide whether a binary speaks Leios at all — the NodeToNode version bump that ships the Leios mini-protocols, and any top-level config flag we may use to gate the feature stack as a whole, in the style of Genesis's `EnableLoEAndGDD` — are compatible with this guideline. They decide whether the code is present, not how it behaves once present. What the guideline rejects is the per-component, per-thread, per-protocol branching that would recreate feature awareness throughout the codebase; that branching is what the ledger replaces.

#### Concrete by default; abstract only where it pays its rent

Every type in the Leios subsystem is monomorphic until a current consumer of the alternative inhabitant can be named. The clearest established example is `m`: consensus code is parametric over `m` because `IO` runs the production node and `IOSim` runs deterministic tests — both committed inhabitants, exercised on every CI run. Everything else stays concrete by default: `LeiosVote` is a BLS-signed record, `Committee`, `Round`, and `EB` are concrete data types, and component handles like `LeiosVoteState m` and `LeiosDbHandle m` are parametric only over `m`. Where consensus already imposes polymorphism — the `blk` block format or `LedgerState blk` at the ledger boundary — a single typeclass or record of accessors consumes `blk` and projects to those concrete types, so the polymorphism does not leak further. When a future consumer is committed (for example, tests that need a fast deterministic signature scheme rather than running real BLS under IOSim), the corresponding parameter is added at the point the second inhabitant exists, not in anticipation.

Don't add parameters for hypothetical consumers: no open type families or class methods for unspecified future formats, no `crypto` or `era` parameters propagated through Leios types without a test or boundary that actually uses them. And don't let `blk` leak past the `LedgerState blk` boundary into Leios-internal types; `LeiosVoteState m` and `LeiosDbHandle m` are the ceiling for component handle parameterization.

The trade-off mirrors structural inertness: we accept a refactor cost later if the protocol generalizes in exchange for fewer type parameters and constraints to read today. Refactoring concrete code is mechanical work that the type checker guides, while every parameter — used or not — appears at every site that mentions the type and shapes how the code reads. This guideline applies to new Leios code; existing consensus interfaces stay as they are. The line between "present need" and "anticipated need" is judgment, not a rule; a present need six weeks away is still present. When introducing polymorphism, the question to answer in the PR description is _which current consumer needs the alternative inhabitant?_ If the answer is "none yet," the answer is "delete the parameter."
