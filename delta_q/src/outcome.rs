...
mod tests {
    ...
    fn test_gossip() {
        let ctx: PersistentContext = "
            IB := gossip(send, (network ->- receive ->- validate), 3000, 15, 0.08, [cpu, net_rcv, net_snd])
        ...
        let cpu = res.load.get("cpu").unwrap();
        for (x, y) in cpu.iter() {
            assert_eq!(y.steps().at(1.0), 1.0, "cpu at {x} is {y}");
        }
        assert_eq!(cpu.at(f32::INFINITY), CDF::top());
        let mut total = 0.0;
        for ((x, y), next) in cpu.graph_iter().tuple_windows() {
            let width = next.0 - x;
            let load = y
                .graph_iter()
                .tuple_windows()
        ...
        assert!((total - 0.1).abs() < 0.0001, "total load is {total}");
    }
}
