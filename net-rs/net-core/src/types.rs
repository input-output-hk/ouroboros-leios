...
//!
//! These types represent the on-wire format of common Cardano structures.
//! Headers and blocks are stored as opaque CBOR bytes — we don't decode
//! era-specific internals at the network layer.
...
/// An era-tagged block header stored as opaque CBOR bytes.
/// We don't decode era-specific header fields at the network layer.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WrappedHeader(pub Vec<u8>);
...
impl<'a> minicbor::Decode<'a, ()> for WrappedHeader {
    fn decode(d: &mut Decoder<'a>, _ctx: &mut ()) -> Result<Self, DecodeError> {
        ...
        Ok(WrappedHeader(raw.to_vec()))
    }
}

// --- BlockBody ---

/// A full block stored as opaque CBOR bytes (including the #6.24 tag wrapper).
/// We don't decode era-specific block fields at the network layer.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn wrapped_header_round_trip() {
        ...
        let header = WrappedHeader(buf.clone());
