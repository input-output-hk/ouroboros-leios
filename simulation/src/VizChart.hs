...
scaledAxis lap rs@(minV, maxV) ps =
 ...
 where
  ...
  tickvs =
    map fromRational $
      steps
        (fromIntegral (Chart._la_nTicks lap))
...
chooseStep nsteps (x1, x2) = minimumBy (comparing proximity) stepVals
 where
  delta = x2 - x1
  mult
    | delta == 0 = 1 -- Otherwise the case below will use all of memory
