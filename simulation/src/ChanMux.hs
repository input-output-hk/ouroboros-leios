...
class MuxBundle bundle where
    ...
    traverseMuxBundle  :: Monad m => (forall a. f a -> m (g a))
                       -> bundle f -> m (bundle g)
...
newMuxChan bearer = do
    ...
    chans <- traverseMuxBundle
               (newMuxChanSingle @bundle bearer sendLock
                                 recvQueuesIx recvQueuesAccum)
               toFromMuxMsgBundle
...
newMuxChanSingle :: forall bundle m a.
                 ...
                 -> m (Chan m a)
newMuxChanSingle bearer sendLock recvQueuesIx recvQueuesAccum
                 ToFromMuxMsg{..} = do
    queue <- newTQueueIO
    i <- atomically $ do
    ...
    return Chan {
      ...
      writeChan = \msg -> let !muxmsg = BearerMsg i (toMuxMsg msg) in
                          withMVar sendLock $ \_ -> writeChan bearer muxmsg
    }

data RecvQueue m mm where
     RecvQueue :: (mm -> a) -> TQueue m a -> RecvQueue m mm

demuxer :: forall bundle m.
           MonadSTM m
        => Chan m (BearerMsg (MuxMsg bundle))
...
data ExampleMsg = MsgFoo { fromMsgFoo :: Int }
