...
data LeiosNodeState m = LeiosNodeState
  ...
  , prunedCertifiedEBStateToVar :: !(TVar m SlotNo)
...
newLeiosNodeState cfg = do
  ...
  prunedCertifiedEBStateToVar <- newTVarIO (toEnum 0)
  ...
  go p = do
    ...
    ebsPruned <-
      atomically $ do
        ...
        -- Update st.prunedCertifiedEBStateToVar with the slot number pruned to.
        writeTVar st.prunedCertifiedEBStateToVar $! pruneTo
    ...
    ebsPruned <-
      atomically $ do
        ...
        let (ebsPruned, prunedEBRelayBuffer) =
              first (fmap snd . RB.values) . flip RB.partition ebRelayBuffer $ \ebEntry ->
                let ebId = (fst ebEntry.value).id
                 in ebId `Set.member` ebIdsToPrune
        writeTVar st.relayEBState.relayBufferVar $! prunedEBRelayBuffer
        -- Prune st.ibsNeededForEBVar by removing the EBs in ebIdsToPrune.
        modifyTVar' st.ibsNeededForEBVar (`Map.withoutKeys` ebIdsToPrune)
