...
export function createForceLayout(
  ...
  return {
    ...
    fixNode: (nodeId: string) => sim.dragStart(nodeId, 0, 0),
    dragNode: (nodeId: string, x: number, y: number) => sim.drag(nodeId, x, y),
    unfixNode: (nodeId: string) => sim.unpinNode(nodeId),
    stop: () => sim.stop(),
  };
}
