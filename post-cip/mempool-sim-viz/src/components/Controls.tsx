...
export function Controls({ onConfigChange, disabled = false, isExpanded, onToggleExpand }: ControlsProps) {
  ...
  useEffect(() => {
    setConfig(prev => {
      ...
      const totalSimTime = next.slots * next.slotDuration;
      if (next.txDuration > totalSimTime) {
        next.txDuration = Math.max(1, totalSimTime);
        changed = true;
      }

      return changed ? next : prev;
    });
  }, [config.nodes, config.slots, config.slotDuration]);
  ...
  const maxTxDuration = Math.max(5, config.slots * config.slotDuration);
    ...
    >
      ...
      {isExpanded && (
        <div className="mt-4">
          ...
          <Section title="Transactions" defaultOpen={false}>
            <Slider
            ...
            <Slider
              ...
              value={config.txDuration}
              min={1}
              max={maxTxDuration}
              unit="s"
              tooltip="Time window over which transactions are randomly submitted"
              onChange={(v) => updateConfig('txDuration', v)}
            />
          </Section>
          ...
          <Section title="Block Production" defaultOpen={false}>
            <Slider
              ...
              min={1}
              max={100}
              tooltip="Number of blocks to produce during the simulation"
            ...
            <Slider
              ...
              max={20}
              unit="s"
              tooltip="Time between consecutive block productions"
              onChange={(v) => updateConfig('slotDuration', v)}
            />
            <Slider
              label="Block Size"
