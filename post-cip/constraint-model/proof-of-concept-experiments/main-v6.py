...
def solve_schedule(data):
    ...
    all_intervals = (
        [task_rb] + list(tasks_va1.values()) + list(tasks_va2.values()) + [task_vt]
    )
    ...
    if status == cp_model.OPTIMAL:
        print_solution(
            solver, data, start_rb, end_rb, tasks_va1, tasks_va2, start_vt, end_vt
        )
    else:
...
def print_solution(solver, data, s_rb, e_rb, t_va1, t_va2, s_vt, e_vt):
    ...
    rb_arr = data["time_rb"]
    print(
        f"{'RB Val':<10} | {rb_arr:<8} | {solver.Value(s_rb):<6} | {solver.Value(e_rb):<6} | Main Gate"
    )
    ...
    # VT Info
    print(
        f"{'VT':<10} | {'-':<8} | {solver.Value(s_vt):<6} | {solver.Value(e_vt):<6} | Final Step"
    )
