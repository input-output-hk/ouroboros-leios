...
def generate_scenario():
    ...
    for i in range(n_eb_tx):
        node_id = start_eh + i
        dag.add_node(node_id, type='EH', 
                     arrival_delay=random.randint(0, 10)) # delta_tb_i
...
def solve_system(params, dag):
    ...
    total_ops = len(dag.nodes) * (params['cost_verify'] + params['cost_apply']) + params['cost_vote']
    ...
    for node in dag.nodes(data=True):
        ...
        v_interval = model.NewIntervalVar(v_start, params['cost_verify'], v_end, f'v_task_{node_id}')
        
        all_intervals.append(v_interval)
        task_metadata[f'v_task_{node_id}'] = {
            'type': 'Verify', 'id': node_id, 'node_type': node_type
        }
        ...
        a_interval = model.NewIntervalVar(a_start, params['cost_apply'], a_end, f'a_task_{node_id}')
        
        all_intervals.append(a_interval)
        apply_end_vars[node_id] = a_end
        task_metadata[f'a_task_{node_id}'] = {
            'type': 'Apply ', 'id': node_id, 'node_type': node_type
        }
...
def process_results(solver, meta, intervals, makespan_var, params):
    ...
    print(f"Scenario: {params['n_cpu']} CPUs | Costs: V={params['cost_verify']} A={params['cost_apply']}")
    print("-" * 65)
