...
def read_scenario(filepath):
    ...
    # Global defaults
    default_sig = params.get('cost_verify')
    default_app = params.get('cost_apply')
    default_rea = params.get('cost_reapply')
    ...
    for tx_hash, attrs in raw_dag.items():
        ...
        if cost_apply is None:
            if default_app is not None:
                cost_apply = default_app
            else:
                raise ValueError(f"Transaction '{tx_hash}' missing 'cost_apply'.")

        cost_reapply = attrs.get('cost_reapply')
        if cost_reapply is None:
            if default_rea is not None:
                cost_reapply = default_rea
            else:
                raise ValueError(f"Transaction '{tx_hash}' missing 'cost_reapply'.")
...
def calculate_statistics(makespan, tasks, params):
    ...
    phases = ['Verify', 'Apply', 'Reapply', 'Vote']
    phase_stats = {}
...
def print_statistics(makespan, stats, file=sys.stderr):
    ...
    print("-" * 40, file=file)
    print(f"{'Phase':<10} | {'Work %':<8} | {'Parallelism':<11}", file=file)
    print("-" * 40, file=file)
    for p, data in stats['phases'].items():
        print(f"{p:<10} | {data['fraction_of_total_work']:<8.1%} | {data['avg_parallelism']:<11.2f}", file=file)
    print("="*40, file=file)
...
def plot_gantt(tasks, makespan, params, filename="gantt.png"):
    ...
    colors = {'Verify': 'tab:blue', 'Apply': 'tab:orange', 'Reapply': 'tab:red', 'Vote': 'tab:green'}
...
def solve_system(params, dag, log_progress=False, gap=None, abs_gap=None):
    ...
    total_sig = sum(d['cost_verify'] for d in active_nodes)
    total_app = sum(d['cost_apply'] for d in active_nodes)
    total_rea = sum(d['cost_reapply'] for d in active_nodes)
    total_ops = total_sig + total_app + total_rea + params['cost_vote']
    ...
    # Pre-calculate base arrival times and max ancestor arrival times
    arrival_times = {}
    for node_id in nx.topological_sort(dag):
        attrs = dag.nodes[node_id]
        if attrs['type'] == 'LG':
            arrival_times[node_id] = 0
        elif attrs['type'] == 'RB':
            arrival_times[node_id] = t_rb + attrs.get('arrival_delay', 0)
        else:
            arrival_times[node_id] = t_eh + attrs.get('arrival_delay', 0)

    max_ancestor_arrival = {}
    for node_id in nx.topological_sort(dag):
        max_arr = arrival_times[node_id]
        for p in dag.predecessors(node_id):
            max_arr = max(max_arr, max_ancestor_arrival[p])
        max_ancestor_arrival[node_id] = max_arr
    ...
    for node_id in nx.topological_sort(dag):
        ...
        cost_verify = attrs['cost_verify']
        cost_apply = attrs['cost_apply']
        cost_reapply = attrs['cost_reapply']
        
        arr_time = arrival_times[node_id]
        max_anc_arr = max_ancestor_arrival[node_id]

        # Sig
        sig_s = model.NewIntVar(arr_time, horizon, f'sig_s_{node_id}')
        sig_e = model.NewIntVar(0, horizon, f'sig_e_{node_id}')
        sig_iv = model.NewIntervalVar(sig_s, cost_verify, sig_e, f'iv_sig_{node_id}')
        all_intervals.append(sig_iv)
        sig_end_vars[node_id] = sig_e
        task_metadata[f'iv_sig_{node_id}'] = {'type': 'Verify', 'id': node_id}

        # App (Needs all upstream data seen; runs in parallel with Sig)
        app_s = model.NewIntVar(max_anc_arr, horizon, f'app_s_{node_id}')
        app_e = model.NewIntVar(0, horizon, f'app_e_{node_id}')
        app_iv = model.NewIntervalVar(app_s, cost_apply, app_e, f'iv_app_{node_id}')
        all_intervals.append(app_iv)
        task_metadata[f'iv_app_{node_id}'] = {'type': 'Apply', 'id': node_id}
        
        # Rea (Needs initial apply done)
        rea_s = model.NewIntVar(0, horizon, f'rea_s_{node_id}')
        rea_e = model.NewIntVar(0, horizon, f'rea_e_{node_id}')
        rea_iv = model.NewIntervalVar(rea_s, cost_reapply, rea_e, f'iv_rea_{node_id}')
        all_intervals.append(rea_iv)
        rea_end_vars[node_id] = rea_e
        task_metadata[f'iv_rea_{node_id}'] = {'type': 'Reapply', 'id': node_id}

        model.Add(rea_s >= app_e)

        # DAG Dependencies (Parent Rea -> Child Rea)
        for parent in dag.predecessors(node_id):
            ...
            if parent_type != 'LG':
                model.Add(rea_s >= rea_end_vars[parent])
    ...
    # Vote must wait for all Rea and Sig tasks
    for node_id in dag.nodes():
        if dag.nodes[node_id]['type'] != 'LG':
            model.Add(vt_start >= rea_end_vars[node_id])
            model.Add(vt_start >= sig_end_vars[node_id])
