...
def run_leiden_clustering(distance_matrix, node_names, D_max, s_min, resolution=1.0):
    """
    ...
    for i in range(M):
        for j in range(i + 1, M):
            ...
            if d <= D_max:
                edges.append((i, j))
                # Aggressive Weighting: Squaring the similarity forces Leiden to heavily
                # prioritize extremely tight pairs over moderately similar pairs.
                similarity_weight = int((D_max - d + 1) ** 2)
    ...
    # RBConfigurationVertexPartition allows us to pass a resolution parameter.
    # Higher resolution = more clusters, smaller sizes, much higher shared transactions.
    ...
    print(
        f"Leiden algorithm finished! Found {len(partition)} total topological communities.\n"
    )
    ...
    print(
        f"Discarded {orphaned_nodes} nodes that belonged to communities too small to qualify.\n"
    )
...
if __name__ == "__main__":
    ...
    RESOLUTION = 1.0  # Higher = tighter clusters, more shared transactions
