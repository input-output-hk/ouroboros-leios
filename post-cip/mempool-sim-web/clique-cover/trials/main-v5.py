...
def optimize_subgraph_facility_location(distance_matrix, node_names, D_max, a, s_min, timeout_seconds=0):
    """
    ...
    if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        ...
        for j in range(M):
            if solver.Value(y[j]) == 1:
                ...
                current_cluster = [node_names[i] for i in valid_i_for_j[j] if solver.Value(x[i, j]) == 1]
                if current_cluster:
...
def visualize_incidence_matrix(A, filename="mempool_vs_transaction.png"):
    ...
    # Mathematically scale the matrix so each cell becomes a 2x2 block
    A_scaled = np.kron(A, np.ones((2, 2)))
    
    # Set up the plot (size dynamically scales to the aspect ratio)
    fig, ax = plt.subplots(figsize=(12, 8))
    
    # Plot using a grayscale colormap. 
    # 'equal' aspect keeps the 2x2 blocks perfectly square.
